...
import pandas as pd
import numpy as np
import matplotlib.pyplot as plt  # 保留此导入，确保有图形库可用
import time
import json
import random
import docx
...
import plotly.graph_objects as go  # 添加 Plotly 导入
import plotly.express as px  # 添加 Plotly Express 导入
...
# 注意：这里缺少了关键的数据来源
# 您需要确保 tests_data_raw 和 holland_jobs_db 已经定义
# 这些应该是通过 load_all_data() 函数加载的数据

# 从缓存中获取数据
try:
    loaded_data = get_data()
    tests_data_raw = loaded_data.get('tests_data_raw', {})
    holland_jobs_db = loaded_data.get('holland_jobs_db', {})
except Exception as e:
    st.error(f"数据加载失败: {str(e)}")
    st.stop()

# 初始化 session_state 中的 answers 字典
for test_cfg in TESTS_CONFIG:
    if test_cfg["id"] not in st.session_state.answers:
        st.session_state.answers[test_cfg["id"]] = []

# 计算总进度和时间
total_questions = sum(t['num_questions'] for t in TESTS_CONFIG)
answered_questions = sum(
    len(st.session_state.answers[t['id']]) for t in TESTS_CONFIG if t['id'] in st.session_state.answers)
...
if not st.session_state.all_tests_completed:
    ...
    # 获取当前测试的题目（取指定数量）
    questions = tests_data_raw.get(current_test_id, [])[:num_q]

    # 检查是否成功获取到题目
    if not questions:
        st.error(f"未能加载 {current_test_name} 的题目数据，请检查数据文件!")
        st.stop()
            ...
            try:
                ...
                fig_mbti.update_layout(
                    polar=dict(
                        radialaxis=dict(
                            visible=True,
                            range=[0, max(mbti_res['scores'].values()) * 1.2] if mbti_res['scores'] else [0, 10]
                            # 设置合适的范围
                        )),
                    showlegend=False,
                    title="MBTI 维度得分",
